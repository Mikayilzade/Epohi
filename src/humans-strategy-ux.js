(function () {
  ...
  function relationCard(gs, civ) {
    ...
    const population=(civ.cities||[]).reduce(function(sum,city){return sum+(city.population||0);},0), power=(civ.units||[]).reduce(function(sum,unit){return sum+((window.EpohiData.UNIT_DEFS[unit.type]||{}).attack||0);},0), army=power>=70?'сильная':power>=30?'средняя':'малая', trade=(civ.technologies||[]).includes('trade')?'известна':'не известна';
    return '<article class="strategy-diplomacy-card ' + relationClass(civ) + '" data-diplomacy-civ="' + civ.civilizationId + '" style="--strategy-civ:' + civ.color + ';--strategy-civ-dark:' + (civ.darkColor || civ.color) + '"><header><span class="strategy-state-seal">' + safeText(civ.symbol || "•") + '</span><div><h3>' + safeText(civ.name) + '</h3><small>' + safeText(civ.diplomacy.temperament || "неизвестный народ") + '</small></div><strong>' + safeText(civ.defeated?'разгромлено':status) + '</strong></header><div class="strategy-relation-meter"><i style="width:' + Math.max(0, Math.min(100, score + 50)) + '%"></i></div><p>Города: '+(civ.cities||[]).length+' · население: '+population+' · армия: '+army+' · Торговля: '+trade+'.</p><div class="strategy-diplomacy-actions">' + actions + '</div></article>';
  }
