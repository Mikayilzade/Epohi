(function () {
  ...
  function relationCard(gs, civ) {
    ...
    if (civ.defeated) actions = '<span class="inline-note">Государство прекратило существование</span>';
    else if (civ.relation === "war") actions += '<button type="button" data-dip-action="peace" data-civ-id="' + civ.civilizationId + '"' + ((gs.resources.gold || 0) < 20 ? " disabled" : "") + '>🕊 Мир · 20 🪙</button>';
    ...
    if (!civ.defeated && civ.relation !== "war") actions += '<button type="button" class="secondary" data-dip-action="gift" data-civ-id="' + civ.civilizationId + '"' + (canGift ? "" : " disabled") + '>🎁 Дар · 10 🪙</button>';
    const population=(civ.cities||[]).reduce(function(sum,city){return sum+(city.population||0);},0), power=(civ.units||[]).reduce(function(sum,unit){return sum+((window.EpohiData.UNIT_DEFS[unit.type]||{}).attack||0);},0), army=power>=70?'сильная':power>=30?'средняя':'малая', trade=(civ.technologies||[]).includes('trade')?'известна':'не известна', route=(gs.tradeRoutes||[]).find(function(item){return item.civId===civ.civilizationId&&item.status==='active'&&item.remainingTurns>0;}), routeHtml=route?'<div class="feedback-trade-status" data-diplomacy-trade-status>⚖️ Торговый путь активен · осталось '+route.remainingTurns+' ход.</div>':'';
    return '<article class="strategy-diplomacy-card ' + relationClass(civ) + '" data-diplomacy-civ="' + civ.civilizationId + '" style="--strategy-civ:' + civ.color + ';--strategy-civ-dark:' + (civ.darkColor || civ.color) + '"><header><span class="strategy-state-seal">' + safeText(civ.symbol || "•") + '</span><div><h3>' + safeText(civ.name) + '</h3><small>' + safeText(civ.diplomacy.temperament || "неизвестный народ") + '</small></div><strong>' + safeText(civ.defeated?'разгромлено':status) + '</strong></header><div class="strategy-relation-meter"><i style="width:' + Math.max(0, Math.min(100, score + 50)) + '%"></i></div><p>Города: '+(civ.cities||[]).length+' · население: '+population+' · армия: '+army+' · Торговля: '+trade+'.</p>'+routeHtml+'<div class="strategy-diplomacy-actions">' + actions + '</div></article>';
  }
